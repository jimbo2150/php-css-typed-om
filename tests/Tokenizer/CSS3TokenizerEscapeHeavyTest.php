...
final class CSS3TokenizerEscapeHeavyTest extends TestCase
{
    public function testHexEscapeInIdentifier()
    {
        ...
        foreach ($tokens as $tok) {
            if ($tok->type === CSS3TokenType::IDENT && $tok->value === 'idA') {
                $found = true;
                break;
            }
        }
    ...
    public function testBackslashNewlineLineContinuationInIdentifier()
    {
        ...
        $found = false;
        foreach ($tokens as $tok) {
            if ($tok->type === CSS3TokenType::IDENT && $tok->value === 'myident') {
                $found = true;
                break;
            }
        }
    ...
    $css = "\\31 a {}";
        ...
        $found = false;
        foreach ($tokens as $tok) {
            if ($tok->type === CSS3TokenType::IDENT && $tok->value === '1a') {
                $found = true;
                break;
            }
        }
    ...
    public function testLongEscapedIdentifierTruncation()
    {
        ...
        $ident = null;
        foreach ($tokens as $tok) {
            if ($tok->type === CSS3TokenType::IDENT) {
                $ident = $tok;
                break;
            }
        }
